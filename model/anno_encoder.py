...
class Anno_Encoder():
		...
		def decode_depth(self, depths_offset, batch_idxs, calib):
			'''
			Transform depth offset to depth
			'''
			fx_list = [t.f_u for t in calib]
			fx_list = torch.tensor(fx_list, dtype=torch.float32,device=batch_idxs.device)
			fx_list = fx_list[batch_idxs]
			f = 500

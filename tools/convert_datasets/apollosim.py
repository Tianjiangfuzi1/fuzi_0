...
def extract_data(ori_json, data_root, tar_path, test_mode=False, sample_step=1):
    ...
    if not os.path.exists(ori_json):
        raise FileNotFoundError(f"{ori_json} does not exist!")
    if os.path.getsize(ori_json) == 0:
        raise ValueError(f"{ori_json} is empty!")
    with open(ori_json, 'r') as anno_obj:
        for line in anno_obj:
            ...
            try:
                ...
                print(f"Error decoding JSON at line: {line}")
                raise e
            cnt_idx += 1
            info_dict = json.loads(line)

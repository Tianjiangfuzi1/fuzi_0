...
def train(model,
    ...
    logger.info("Data shuffle: {}".format(cfg.data_shuffle))
    ...
    # The specific dataloader settings(Anchor3DLane)
    train_loader_cfg = {**loader_cfg, **cfg.data.get('train_dataloader', {})}
    data_loaders = [build_dataloader(dataset, **train_loader_cfg) for dataset in datasets]

    # MonoFlex dataloader setting
    # data_loader_monoflex = make_data_loader(cfg, is_train=True)
    # put model on devices
    if distributed:
        print("*****************distributed******************")
        find_unused_parameters = cfg.get('find_unused_parameters', False)  #没有找到，返回false
        ...
        # check_path = '/home2/wr21125091/Anchor3DLane/output/openlane/anchor3dlane/20250212/iter_170000.pth'
    ...
    if cfg.optimizer.type == 'Adam':
        optimizer = torch.optim.Adam(parameters, lr=cfg.optimizer.lr)
    ...
    # cfg.workflow.append(('train', 1000000))
...
def main():
    ...
    # set cudnn_benchmark
    if cfg.get('cudnn_benchmark', False): # 没找到返回false  找到执行下方内容
        print("**************************")

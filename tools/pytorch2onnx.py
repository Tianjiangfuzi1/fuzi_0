...
def symbolic_dcnv2(g, input, offset, mask, weight, bias, stride, padding, dilation, deformable_groups):
    return g.op("DCNv2", input, offset, mask, weight, bias, stride, padding, dilation, deformable_groups)
register_custom_op_symbolic("torchvision::DCNv2", symbolic_dcnv2, 11)
...
def _prepare_input_img(img_path,
    ...
    test_pipeline[1]['keep_ratio'] = False
    ...
    results = {}
    results['img_info'] = {}
    results['filename'] = data['img']
    results['img_info']['filename'] = results['filename']
    front_path = '/root/autodl-tmp/data/OpenLane/images/'
    relative_path = os.path.relpath(data['img'], front_path)
    base, ext = os.path.splitext(relative_path)
    id_path = base + '.pkl'
    pkl_path = '/root/autodl-tmp/data/OpenLane/cache_dense/'
    results['img_info'] = os.path.join(pkl_path, id_path)
    results['ori_filename'] = results['filename']
    results['ori_shape'] = (1280, 1920)
    results['flip'] = False
    results['flip_direction'] = None
    with open(results['img_info'], 'rb') as f:
        obj = pickle.load(f)
        results.update(obj)
    results['img_metas'] = {'ori_shape': results['ori_shape']}
    results['gt_project_matrix'] = projection_g2im_extrinsic(results['gt_camera_extrinsic'],
                                                             results['gt_camera_intrinsic'])
    results['gt_homography_matrix'] = homography_g2im_extrinsic(results['gt_camera_extrinsic'],
                                                                results['gt_camera_intrinsic'])
    results['img'] = data['img']
    data = test_pipeline(results)
    imgs = data['img']
    # img_metas = [i.data[0] for i in data['img_metas']]
...
def pytorch2onnx(model,
    ...
    # img_list = [img[None, :] for img in imgs.data]
    # img_meta_list = [[img_meta] for img_meta in img_metas]
    img_list = [imgs.data[None, :, :, :]]
    img_meta_list = [img_metas]
    # update img_meta
    img_list = [img.unsqueeze(0) if img.dim() == 3 else img for img in img_list]
    ...
    model.eval()
    img_list[0] = img_list[0].to(device)
    for name, buf in model.named_buffers():
        if buf.device.type != device.type:
            print(f"Warning: Buffer {name} is on {buf.device} but expected {device}. Moving it.")
            buf.data = buf.data.to(device)
    for name, param in model.named_parameters():
        if param.device.type != device.type:
            print(f"Warning: Parameter {name} is on {param.device} but expected {device}. Moving it.")

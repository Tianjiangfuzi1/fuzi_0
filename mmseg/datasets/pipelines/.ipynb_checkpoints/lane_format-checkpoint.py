...
class LaneFormat(object):
    ...
    def __call__(self, results):
        ...
        if 'lidartoimage' in results:
            results['lidartoimage'] = DC(to_tensor(results['lidartoimage'][None, ...].astype(np.float32)), stack=True)
        if 'class' in results:
            results['class'] = DC(to_tensor(results['class'][None, ...].astype(np.float32)), stack=True)
        if 'bbox_2d' in results:
            results['bbox_2d'] = DC(to_tensor(results['bbox_2d'][None, ...].astype(np.float32)), stack=True)
        if 'dimensions' in results:
            results['dimensions'] = DC(to_tensor(results['dimensions'][None, ...].astype(np.float32)), stack=True)
        if 'center' in results:
            results['center'] = DC(to_tensor(results['center'][None, ...].astype(np.float32)), stack=True)
        if 'rotation_y' in results:
            results['rotation_y'] = DC(to_tensor(results['rotation_y'][None, ...].astype(np.float32)), stack=True)
        return results

...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, from_video=False, use_right_cam=False):
        ...
        # self.P[0][0] = self.P[0][0] * 0.25
        # self.P[1][1] = self.P[1][1] * 0.25
        # self.P[0][2] = self.P[0][2] * 0.25
        # self.P[1][2] = self.P[1][2] * 0.25
...
def draw_projected_box3d(image, qs, color=None, cls=None, thickness=1, draw_orientation=False, draw_height_text=False,
    ...
    if draw_orien_surface:
        ...
        orien_mask = np.zeros((image.shape), dtype=np.float32)
        # orien contours
        contours = qs[[0, 1, 5, 4], :].astype(np.int64)
        # contours = qs[[0, 1, 5, 4], :].astype(np.float64)
        orien_mask = cv2.fillPoly(orien_mask, pts=[contours], color=color)
        orien_mask = orien_mask.astype(np.float32) / 255.0
    ...
    return image
# def draw_projected_box3d(image, qs, color=(0, 60, 255), thickness=1):
#     '''
#     qs: °üº¬8¸ö3D±ß½ç¿ò½Çµã×ø±êµÄÊý×é, ÐÎ×´Îª(8, 2)¡£Í¼Ïñ×ø±êÏÂµÄ3D¿ò, 8¸ö¶¥µã×ø±ê¡£
#     '''
#     ''' Draw 3d bounding box in image

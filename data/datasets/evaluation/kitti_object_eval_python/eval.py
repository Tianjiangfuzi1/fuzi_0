...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None, metric='R40'):
    ...
    # mAPbbox, mAPbev, mAP3d, mAPaos = do_eval(dt_annos, dt_annos, current_classes,
    #                         min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, metric=metric)
    ret_dict = {}
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            ret_dict['{}_3d_{:.2f}/easy'.format(class_to_name[curcls], min_overlaps[i, 1, j])] = mAP3d[j, 0, i]
            ret_dict['{}_3d_{:.2f}/moderate'.format(class_to_name[curcls], min_overlaps[i, 1, j])] = mAP3d[j, 1, i]
            ret_dict['{}_3d_{:.2f}/hard'.format(class_to_name[curcls], min_overlaps[i, 1, j])] = mAP3d[j, 2, i]

            ret_dict['{}_bev_{:.2f}/easy'.format(class_to_name[curcls], min_overlaps[i, 2, j])] = mAPbev[j, 0, i]
            ret_dict['{}_bev_{:.2f}/moderate'.format(class_to_name[curcls], min_overlaps[i, 2, j])] = mAPbev[j, 1, i]
            ret_dict['{}_bev_{:.2f}/hard'.format(class_to_name[curcls], min_overlaps[i, 2, j])] = mAPbev[j, 2, i]

            ret_dict['{}_image/easy'.format(class_to_name[curcls])] = mAPbbox[j, 0, 0]
            ret_dict['{}_image/moderate'.format(class_to_name[curcls])] = mAPbbox[j, 1, 0]
            ret_dict['{}_image/hard'.format(class_to_name[curcls])] = mAPbbox[j, 2, 0]
